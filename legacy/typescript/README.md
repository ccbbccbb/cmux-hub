...
This repository now includes a Rust-first rebuild aimed at dramatically improving startup latency, diff throughput, and runtime stability.

### Repository layout

- `src/`: Rust application source (CLI, git integration, HTTP/WebSocket server)
- `web/`: Rust-served frontend (vanilla JS port of the diff/commit/actions interface)
- `legacy/typescript/`: archived TypeScript implementation for parity checks and migration reference
